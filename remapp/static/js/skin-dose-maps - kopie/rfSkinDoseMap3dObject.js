...
function skinDoseMap3dObject(skinDoseMap3dCanvasName, colourScaleName) {
    ...
    this.draw = function () {
        ...
        for (i = 0; i < torsoStart; i++) {
            for (j = 0; j < _this.phantomFlatWidth*2+_this.phantomCurvedEdgeWidth; j++) {
                ...
                if (j==_this.phantomFlatWidth) {j+= _this.phantomCurvedEdgeWidth}
            }
        }
/*
        for (i = 0; i < torsoStart; i++) {
            for (j = _this.phantomFlatWidth+_this.phantomCurvedEdgeWidth; j < _this.phantomFlatWidth*2+_this.phantomCurvedEdgeWidth; j++) {
                currentDose = _this.skinDoseMap[j * (_this.phantomHeight + _this.phantomHeadHeight) + i]*0;
                scaledDose = currentDose - (_this.windowLevel - (_this.windowWidth / 2.0));
                if (scaledDose < 0) {scaledDose = 0;}
                if (scaledDose > _this.windowWidth) {scaledDose = _this.windowWidth;}
                newColour = _this.colourScale(scaledDose / _this.windowWidth).rgb();
     ...
     */
    this.initialise = function (skinDoseMap, phantomFlatWidth, phantomCurvedEdgeWidth, phantomHeight, phantomCurvedRadius, phantomHeadHeight, phantomHeadCircumference) {
        var _this = this;
        ...
        _this.phantomHeadCircumference = phantomHeadCircumference
...
*/
        ...
        headData = new Uint8Array(2*(_this.phantomFlatWidth)*_this.phantomHeadHeight*4)
        ...
        dataTextureHead = new THREE.DataTexture(headData, 2*(_this.phantomFlatWidth), _this.phantomHeadHeight, THREE.RGBAFormat)
        ...
        var materials = [materialBack, materialLeft, materialFront, materialRight, endMaterial, endMaterial, endMaterial, endMaterial, endMaterial, endMaterial, materialHead, endMaterial];
        ...
        geometry = new THREE.CylinderGeometry(_this.phantomFlatWidth/2, _this.phantomFlatWidth/2, _this.phantomHeadHeight, 2*_this.phantomFlatWidth, 1, true, -Math.PI/2, 2*Math.PI );
        mesh = new THREE.Mesh(geometry);
