...
class Phantom_3:
    ...
    def __init__(self, origin, scale=1, mass=73.2, height=178.6, patPos="HFS"):
        ...
        if patPos == "HFS":
            ...
            patPosZ = 1.
            patPosY = 1.
            origin[1] = origin[1] #- 24 * height / refHeight  # subtractie hoofdlengte
        elif patPos == "FFS":
            prone = False
            patPosZ = 1.
            patPosY = -1.
        ...
        roundFlat = 2 * round(flatWidth / 2, 0) + 1
        flatSpacing = flatWidth / roundFlat

        headHeight = round(24 * height / refHeight)
        radiusHead = roundFlat / 2
        headCircumference = 2 * math.pi * radiusHead

        # The three properties were added by DJP to describ
        ...
        self.phantom_head_circumference = headCircumference
        self.phantom_head_height = headHeight

        self.width = int(2 * roundCircumference + 2 * roundFlat)
        self.height = int(round(torso, 0))
        self.phantomType = "3d"
        self.phantomMap = np.empty((self.width, self.height + self.phantom_head_height), dtype=object)
        self.normalMap = np.empty((self.width, self.height + self.phantom_head_height), dtype=object)
        ...
        while not it.finished:
            ...
            angleStepHead = math.pi / roundFlat
            zOffset = -origin[2]

            if row_index < transition1 and col_index > self.phantom_head_height:
                ...
                #curve positive
            elif row_index >= transition1 and row_index < transition2 and col_index > self.phantom_head_height:
                myY = (col_index) * patPosY
            ...
            elif row_index >= transition2 and row_index < transition3 and col_index > self.phantom_head_height:
                myZ = zOffset * patPosZ
                ...
                myY = col_index * patPosY
                normal = Segment_3(np.array([myX, myY, myZ - patPosZ]), np.array([myX, myY, myZ]))
            elif row_index >= transition3 and row_index < transition4 and col_index > self.phantom_head_height:
                myY = col_index * patPosY
                ...
                myZ = (zOffset - radius * math.cos(
                    angleStep * (row_index - round(transition3, 0) + 1)) + radius) * patPosZ
                ...
                normal = Segment_3(np.array([normalX, myY, normalZ]), np.array([myX, myY, myZ]))
            elif row_index >= transition4 and col_index > self.phantom_head_height:
                myZ = (2. * radius + zOffset) * patPosZ
                myX = (row_index - self.width) * flatSpacing - (roundFlat / 2.) + round(roundFlat / 2., 0)
                myY = col_index * patPosY
                normal = Segment_3(np.array([myX, myY, myZ + patPosZ]), np.array([myX, myY, myZ]))

            elif row_index < transition1 and col_index <= self.phantom_head_height:#q1
                myY = (col_index) * patPosY
                #myX = row_index * flatSpacing - (roundFlat / 2.) + round(roundFlat / 2., 0)
                myX = radiusHead * math.sin(angleStepHead * (row_index )) - (roundFlat / 2.) + round(roundFlat / 2., 0)
                myZ = zOffset + radiusHead * (math.cos(angleStepHead * row_index)  ) * patPosZ + self.phantom_depth/2
                normalX = myX + math.sin(angleStepHead * (row_index ))
                #normalX = myX
                normalZ = myZ + patPosZ * math.cos(angleStepHead * (row_index + 0))
                normal = Segment_3(np.array([normalX, myY, normalZ]), np.array([myX, myY, myZ]))
            elif row_index >= transition2 and row_index < transition3 and col_index <= self.phantom_head_height: #h
                myY = (col_index) * patPosY
                myX = radiusHead * math.cos(angleStepHead * (row_index - round(transition2, 0) +1 )) - (roundFlat / 2.) + round(roundFlat / 2., 0)
                #myX = flatWidth - (row_index - roundCircumference) * flatSpacing + ((roundFlat / 2.) - round(
                    #roundFlat / 2., 0)) * (row_index - roundCircumference) / abs(row_index - roundCircumference)
                myZ = zOffset + radiusHead * (math.sin(math.pi + angleStepHead * (row_index - round(transition2, 0) + 1)) + 0)* patPosZ + self.phantom_depth/2
                normalX = myX + math.cos(angleStepHead * (row_index - round(transition2 , 0) +1))
                #normalX = myX
                normalZ = myZ - patPosZ * math.sin(angleStepHead * (row_index - round(transition2, 0) + 1))
                normal = Segment_3(np.array([normalX, myY, normalZ]), np.array([myX, myY, myZ]))
            elif row_index >= transition4 and col_index <= self.phantom_head_height:#q2
                myY = (col_index) * patPosY
                myX = -radiusHead * math.cos(angleStepHead * (row_index - round(transition4, 0) + 1)) - (roundFlat / 2.) + round(roundFlat / 2., 0)
                #myX = (row_index - self.width) * flatSpacing - (roundFlat / 2.) + round(roundFlat / 2., 0)
                myZ = zOffset + radiusHead * math.sin(angleStepHead * (row_index - round(transition4, 0) + 1)) * patPosZ + self.phantom_depth/2
                normalX = myX - math.cos(angleStepHead * (row_index - round(transition4, 0) + 0))
                #normalX = myX
                normalZ = myZ + patPosZ * math.sin(angleStepHead * (row_index - round(transition2, 0) + 1))
                normal = Segment_3(np.array([normalX, myY, normalZ]), np.array([myX, myY, myZ]))
            else:
                myY, myX, myZ = [-100000, -100000, -100000]
                normal = Segment_3(np.array([-100000, -100000, -100000]), np.array([myX, myY, myZ]))
...
class SkinDose:
    ...
    def addDose(self, skinmap_result, dap):
        if len(self.doseArray) == 0:
            self.doseArray = skinmap_result[0]
            self.totalDose = skinmap_result[0]
            self.hitCount = [skinmap_result[1] * 100]
            if np.sum(skinmap_result[0]):
                self.dapcount = dap
        else:
            self.doseArray = np.dstack((self.doseArray, skinmap_result[0]))
            self.totalDose = self.totalDose + skinmap_result[0]
            self.hitCount = self.hitCount + [skinmap_result[1] * 100]
            if np.sum(skinmap_result[0]):
                self.dapcount = self.dapcount + dap
