...
from tools.check_coordinate_system import check_coordinate_system
...
def make_skin_map(study_pk=None):
    ...
    if study_pk:
        ...
        my_exp_map = calc_exp_map.CalcExpMap(phantom_type='3D', patPos=patPos,
                                             ...
                                             table_thick=0.5, table_width=40.0, table_length=200,
                                             matt_thick=4.0)


        offset_y = check_coordinate_system(study)
        ...
        for irrad in study.projectionxrayradiationdose_set.get().irradeventxraydata_set.all():
            ...
            try:
                delta_y = float(irrad.irradeventxraymechanicaldata_set.get().doserelateddistancemeasurements_set.get(
                                                                        ).table_lateral_position) / 10.0 + offset_y
            except (ObjectDoesNotExist, TypeError):
                delta_y = 0.0
            try:
            ...
            print('tube coordinate: ' + str(delta_x) + '   ' + str(delta_y) + '   ' + str(delta_z))
            print('primary angle: ' + str(angle_x) + '  secondary angle: ' + str(angle_y))
...
#            my_exp_map.my_dose.totalDose[]
        except ValueError:
            pass
        try:
            from remapp.models import SkinDoseMapResults
            ...
            SkinDoseMapResults(general_study_module_attributes=study,
                               ...
                               peak_skin_dose=np.max(my_exp_map.my_dose.totalDose),
                               hit_fraction=np.sum(my_exp_map.my_dose.hitCount)/my_exp_map.num_views,
                               dap_percentage_contributed=my_exp_map.my_dose.dapcount/np.float(study.total_dap)*100
                               ).save()
        except:
            pass
        ...
        return_structure = {
            ...
            'height': my_exp_map.phantom.height + my_exp_map.phantom.phantom_head_height ,
            'phantom_width': my_exp_map.phantom.phantom_width,
            'phantom_height': my_exp_map.phantom.phantom_height,
            'phantom_depth': my_exp_map.phantom.phantom_depth,
            ...
            'phantom_head_circumference': my_exp_map.phantom.phantom_head_circumference,
