...
from tools.openskin.geomfunc import *
import time
from decimal import *
...
def skinMap(xRay, phantom, area, refAK, kV, filterCu, Dref, tableLength, tableWidth, transmission,
    ...
    skinMap = np.zeros((phantom.width, phantom.height + phantom.phantom_head_height), dtype=np.dtype(Decimal))
    focus = xRay.source
    ...
    while not it.finished:
        ...
        it.iternext()
    hitcount = hitcount/it.itersize


    return [skinMap, hitcount]
...
def rotational(xRay, startAngle, endAngle, frames, phantom, area, refAK, kV, filterCu, Dref, tableLength, tableWidth,
    ...
    myDose, hitcount = skinMap(xRay, phantom, area, refAK / frames, kV, filterCu, Dref, tableLength, tableWidth, transmission,
                     tableMattressThickness)
    hitcount_frames=0
    for i in range(1, frames - 1):
        xRay = rotateRayY(xRay, rotationAngle)
        myDose_temp, hitcount_temp = skinMap(xRay, phantom, area, refAK / frames, kV, filterCu, Dref, tableLength, tableWidth,
                                  transmission, tableMattressThickness)
        myDose += myDose_temp
        hitcount += hitcount_temp
    hitcount = hitcount/frames
    return myDose, hitcount
