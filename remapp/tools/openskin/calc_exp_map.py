import tools.openskin.geomclass as geomclass
import tools.openskin.geomfunc as geomfunc
import tools.openskin.skinMap as skinMap
...
class CalcExpMap(object):
    ...
    def __init__(self, phantom_type=None, patPos=None,
        ...
        if self.phantom_type == 'flat':
            ...
            # The 1 is the scale
            self.phantom = geomclass.Phantom("flat", [17, 0, 0], self.table_width, self.table_length, 10, 1)
            self.matt_thick = 0.0
    ...
    def add_view(self,
        ...
        #print('xray angle x' + str(angle_x))
        #print('xray angle y' + str(angle_y))
        ...
        if 'Rotational' in run_type:
            calc_result = skinMap.rotational(x_ray, angle_x, end_angle, int(frames), self.phantom, area, ref_ak,
                                                    kvp, filter_cu, d_ref,
                                                    self.table_length, self.table_width,
                                                    self.table_trans,
                                                    self.table_thick + self.matt_thick)
        else:
            calc_result = skinMap.skinMap(x_ray, self.phantom, area, ref_ak, kvp, filter_cu, d_ref,
                                                 self.table_length, self.table_width,
                                                 self.table_trans,
                                                 self.table_thick + self.matt_thick)
        self.my_dose.addDose(calc_result, dap)
